...
(() => {
    ...
    root.innerHTML = `
      <div class="miniCal">
        <div class="miniCal__top">
          <div class="miniCal__tabs" role="tablist" aria-label="월 선택">
            ${months
              .map((m, i) => {
                ...
                return `
                  <button
                    class="miniCal__tab ${isOn ? "is-active" : ""}"
                    data-tab="${key}"
                    type="button"
                  >
                    ${m.getMonth() + 1}월
          ...
          <div class="miniCal__title">
            ${targetMonthDate.getFullYear()}.${pad2(targetMonthDate.getMonth() + 1)}
          </div>
        </div>
        ...
        <div class="miniCal__grid">
          ${cells
            .map((c) => {
              ...
              return `
                <div class="miniCal__cell ${c.isOtherMonth ? "is-other" : ""} ${statusClass} ${todayClass}" data-ymd="${c.ymd}">
  ...
  async function main() {
    ...
    for (const r of metaRows) {
      ...
      const next_open_date = (r.next_open_date || "").trim();
...
(() => {
  ...
  const NOTICE_CSV_URL =
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vTNJUl8qQYgFvPX5FghbjrApLUGLR7tou-ufaOlOrMh4aWlI757ec3Sn64vGVLo7QxaKTKR50x8tI_Z/pub?gid=2027436448&single=true&output=csv";
        ...
        <ul class="noticeList" role="list">
          ${items
            .map((it) => {
              ...
              return `
                <li class="noticeItem">
                  <div class="noticeItem__icon" aria-hidden="true">${escapeHTML(icon)}</div>
                  <div class="noticeItem__desc">${sanitizeDescAllowEm(it.desc)}</div>
                </li>
              `;
            })
            .join("")}
...
(() => {
  ...
  function openModal(id, title) {
    ...
    */
    ...
    modal.innerHTML = `
      ...
      <div class="pofModal__panel" role="dialog" aria-modal="true" aria-label="${escapeHTML(title || "영상 재생")}">
        ...
        ${title ? `<div class="pofModal__title">${escapeHTML(title)}</div>` : ""}
      </div>
    `;
  ...
  function render(data) {
    ...
    function mount(cat) {
      ...
      stage.classList.toggle("is-single", base.length === 1);
          ...
          ${items
            .map((it) => {
              ...
              return `
                ...
                >
                  <div class="pofThumb">
                    <img class="pofThumb__img" alt="${escapeHTML(it.title)} 썸네일" loading="lazy">
                    <span class="pofThumb__play" aria-hidden="true">▶</span>
                  </div>
                  <div class="pofMeta">
                    <div class="pofTitle">${escapeHTML(it.title)}</div>
                    ${tags ? `<div class="pofTags" aria-label="태그">${tags}</div>` : ""}
                  </div>
                </button>
              `;
            })
    ...
    root.addEventListener("click", (e) => {
      ...
      if (nav) {
        if (
          stage.classList.contains("is-empty") ||
          stage.classList.contains("is-single")
        ) {
          return;
        }
    ...
    rail.addEventListener("scroll", () => {
      ...
      const track = rail.querySelector(".pofTrack");
      if (!track) return;

      const base = data.filter((v) => v.category === active);
...
(function(){
  ...
  function bindParentAnchorScroll(){
    ...
    links.forEach((link) => {
      if (link.dataset.parentScrollBound === '1') return;
      link.dataset.parentScrollBound = '1';
